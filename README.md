...
- fls.py
- IntCuad.py          
The main is `fctrz`, the program try to get a formula for an `2D` integral on a triangle of something like x^m*y^n by simplex optimization methods, obiusly for bigger indexes m and n the process will be tedius and dificult for the machine inclusive.
for example : 
